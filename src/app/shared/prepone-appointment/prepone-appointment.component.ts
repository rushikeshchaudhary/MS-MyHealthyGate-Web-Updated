...
export class PreponeAppointmentComponent implements OnInit {
  ...
  onSlotSelect(slot: any) {
    ...
    // });
    this.confirmation.startTime = slot.startTimeftm;
    this.confirmation.endTime = slot.endTimeftm;
  ...
  onDateChange(event: any):any {
    ...
    this.usersService
      .getStaffAvailabilityByLocation(this.staffId, this.locationId, selDate, this.appointmentMode)
        ...
        if (response.statusCode == 200) {
          this.schedulerService
            ...
            .subscribe((response: any) => {
              ...
              if (availibiltyResponse.data.length > 0) {
                availibiltyResponse.data.forEach((x: { isSelected: any; }) => {
                  ...
                  this.providerAvailiabilitySlots.push({
                    ...
                    endTimeftm: this.convertHours(timeObj.endTime).trim(),
  ...
  getNearestDate() {
    ...
    this.unreservedproviderAvailiabilitySlots = this.unreservedproviderAvailiabilitySlots.filter((x: { isAvailable: boolean; }) => x.isAvailable == true);
