...
import { PatientRadioData } from "../models/sidebarInfo";
import { TranslateService } from "@ngx-translate/core";

@Component({
  selector: "app-radiology-referral-modal",
  templateUrl: "./radiology-referral-modal.component.html",
  styleUrls: ["./radiology-referral-modal.component.css"],
})
export class RadiologyReferralModalComponent implements OnInit {
  ...
  constructor(
    private dialogModalRef: MatDialogRef<RadiologyReferralModalComponent>,
  ...
  removeInput(index: number) {
    const array=this.form.get("inputsUrlArray") as FormArray;
  ...
  viewDocument = (act: { id: number; }) => {
    if (act) {
      this.clientService.downloadLabFile(act.id).subscribe((response: any) => {
        if (response != null) {


          let newBlob = new Blob([response], { type: response.type });
