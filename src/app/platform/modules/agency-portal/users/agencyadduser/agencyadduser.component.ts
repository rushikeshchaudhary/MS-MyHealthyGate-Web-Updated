...
@Component({
  selector: 'app-agencyadduser',
  templateUrl: './agencyadduser.component.html',
  styleUrls: ['./agencyadduser.component.css']
})
export class AgencyadduserComponent implements OnInit {
  ...
  getMasterData(value: any, type: any = 0) {
    ...
    this.usersService
      .getMasterData(
        "masterStaff,masterLocation,MASTERROLES,masterCountry,masterGender,masterState,masterDegree,MASTERTAGSFORSTAFF,PAYROLLGROUP,MASTERSPECIALITY,MASTERPROVIDERCARECATEGORY,MASTERTAXONOMY,MASTERSTAFFSERVICE,MASTERPROVIDEROLECATEGORY",
        true,
        this.prvSpeciality
      )
      .subscribe((response: any) => {
    ...
    this.userRoleService.getAll(filterModel).subscribe(
      (response) => {
        ...
        if (response.data != null && response.statusCode==200) {
          ...
          this.masterProviderCareCategory = response.data.map((r:any) => ({
            id: r.id,
            value: r.roleName,
            key: r.id.toString()
          }));
  ...
  checkAboutMeLength() {
    var aboutLength = this.userForm.controls["aboutMe"].value.length;
    if (aboutLength > 500) {
      console.log("sub");
  ...
  onSubmit(event: any) {
    ...
    if (!this.userForm.invalid) {
      ...
      var result = this.masterProviderCareCategory.filter((r: { id: any; }) => r.id == formValues.staffUserRole);
      ...
      this.usersService.create(this.userModel).subscribe((response: any) => {
        ...
        if (response.statusCode == 200) {
          ...
          this.route.navigate(["web/manage-users/adduser"], {
            queryParams: {
  ...
  onBackHandler = () => {
    this.route.navigate(["web/manage-users/users"]);
  };
  getUserPermissions() {
