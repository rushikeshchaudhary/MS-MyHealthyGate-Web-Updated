...
export class UserTimeSheetSheetViewComponent implements OnInit {
  ...
  constructor(private userTimeSheetViewService: UserTimeSheetViewService, private translate: TranslateService) {
  ...
  renderTotalData(responseArray: any): any {
    ...
    if (responseArray && responseArray.length) {
      responseArray.map((Obj: { sunday: number; monday: number; tuesday: number; wednesday: number; thursday: number; friday: number; saturday: number; total: number; }, index: any) => {
