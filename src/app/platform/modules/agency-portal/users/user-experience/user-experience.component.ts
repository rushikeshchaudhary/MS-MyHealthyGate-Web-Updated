...
import { AddEditUserExperienceComponent } from "./add-edit-user-experience/add-edit-user-experience.component";
...
export class UserExperienceComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log(parseInt(this.staffId));
  }
  ...
  onPageOrSortChange = (actionObj: { pageNumber: number; pageSize: number; sort: string; order: string; }) => {
  ...
  onTableActionClick = (e: { action?: any; data: any; }) => {
    console.log(e);
    if (e.action == "delete") {
  ...
  downloadUserDocument(value: { action?: any; data: any; }) {
    // this.clientService
  ...
  createModal = (type: any, qualificationData: any) => {
    ...
    let documentModal;
    documentModal = this.dialogModal.open(AddEditUserExperienceComponent, {
      data: staffData,
    });
